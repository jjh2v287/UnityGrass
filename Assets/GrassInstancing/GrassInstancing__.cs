...
[ExecuteInEditMode]
public class GrassInstancing__ : MonoBehaviour
{
    [Range(1, 100_000)]
    ...
    private async UniTask InitTreeNodeData()
    {
        for(int icount=0; icount < DataNode.Count; icount++)
        {
            ...
            if (!iskdtree)
            {
                ...
                node.IndexPos = new List<List<IndexVector4>>();
                for (int xx = 0; xx < (int)node.bounds.size.x; xx++)
                {
                    node.IndexPos.Add(new List<IndexVector4>());
                    for (int zz = 0; zz < (int)node.bounds.size.z; zz++)
                    {
                        node.IndexPos[xx].Add(new IndexVector4());
                        node.IndexPos[xx][zz].pos = new Dictionary<int, Vector4>();
                    }
                }
            }
            else
            ...
            for (int i = 0; i < instanceCount; i++)
            {
                ...
                if (!iskdtree)
                {
                    ...
                    int xx = (int)Mathf.Abs(boundsMin.x - pos.x);
                    if (xx >= (int)node.bounds.size.x)
                        xx = (int)node.bounds.size.x - 1;
                    int zz = (int)Mathf.Abs(boundsMax.z - pos.z);
                    if (zz >= (int)node.bounds.size.z)
                        zz = (int)node.bounds.size.z - 1;

                    // 좌표를 인덱스화한 데이터 잔디 지울때 사용하는 데이터
                    node.IndexPos[xx][zz].pos.Add(i, pos);
                }
                else
                {
                    node.Hash.Add(code, pos);
    ...
    private void CheckPosition(QtreeNode node)
    {
        if(node != null && node.positions != null)
        {
            bool bufferReset = false;
            foreach (var pos in node.positions.ToList())
            {
                float f1 = GetWorldHeightOnTerrain(terrain, transform.position + new Vector3(pos.Value.x, 0, pos.Value.z));
                //if (MathF.Abs(pos.Value.y - f1) > 0.5f)
                {
                    node.positions[pos.Key] = new Vector4(pos.Value.x, f1, pos.Value.z, pos.Value.w);
                    bufferReset = true;
                }
            }
    ...
    public void RemoveGrass(QtreeNode node, Vector3 hitpos, float Radius)
    {
        ...
        else
        {
            ...
            if (intersects)
            {
                ...
                if (!iskdtree)
                {
                    int startX = (int)Mathf.Abs(node.bounds.min.x - hit2d.x) - 1;
                    int startZ = (int)Mathf.Abs(node.bounds.max.z - hit2d.y) - 1;
                    if (startX < 0)
                        startX = 0;
                    if (startZ < 0)
                        startZ = 0;

                    int endX = (int)Mathf.Abs(node.bounds.min.x - hit2d.x) + 1;
                    int endZ = (int)Mathf.Abs(node.bounds.max.z - hit2d.y) + 1;
                    if (endX >= (int)node.bounds.size.x)
                        endX = (int)(node.bounds.size.x - 1);
                    if (endZ >= (int)node.bounds.size.z)
                        endZ = (int)(node.bounds.size.z - 1);

                    for (int xx = startX; xx <= endX; xx++)
                    {
                        for (int zz = startZ; zz <= endZ; zz++)
                        {
                            foreach (int iKey in node.IndexPos[xx][zz].pos.Keys.ToList())
                            {
                                Vector2 node2d = new Vector2(node.IndexPos[xx][zz].pos[iKey].x, node.IndexPos[xx][zz].pos[iKey].z);
                                float dist = Vector2.Distance(node2d, hit2d);
                                if (dist <= Radius)
                                {
                                    node.positions.Remove(iKey);
                                    node.IndexPos[xx][zz].pos.Remove(iKey);
                                    bufferReset = true;
                                }
                            }
                        }
                    ...
                    /*for (int icount = node.positions.Count - 1; icount >= 0; icount--)
                    {
                        ...
                        if (dist <= Radius)
                        {
                            node.positions.RemoveAt(icount);
                            bufferReset = true;
                        }
                    }*/
                    if (bufferReset)
        ...
        if (rootNode != null)
        {
            if (Input.GetKey(KeyCode.B))
            {
                ...
                if (Physics.Raycast(ray, out terrainHit, 200f, paintMask.value))
                {
                    ...
                    RemoveGrass(rootNode, hitPos + new Vector3(5,0,0), brushSize);

                    RemoveGrass(rootNode, hitPos + new Vector3(0, 0, 5), brushSize);
    ...
    private float GetWorldHeightOnTerrain(Terrain terrain, Vector3 worldPos)
    {
        ...
        //worldY = terrain.terrainData.GetHeight((int)worldPos.x, (int)worldPos.z);
        return worldY;
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        /*if (rootNode != null)
        {
            foreach (var draw in rootNode.children)
            {
                foreach (var pos in draw.positions)
                {
                    RaycastHit hit;
                    if (Physics.Raycast(new Vector3(pos.Value.x, 100, pos.Value.z), new Vector3(0, -1, 0), out hit, 200))
                    {
                        if (MathF.Abs(pos.Value.y - hit.point.y) > 0.5f)
                        {
                            Handles.color = Color.white;
                            Handles.DrawWireCube(pos.Value, new Vector3(0.1f, 0.1f, 0.1f));
                            Handles.color = Color.red;
                            Handles.DrawWireCube(hit.point, new Vector3(0.1f, 0.1f, 0.1f));

                            float f1 = GetWorldHeightOnTerrain(terrain, transform.position + new Vector3(pos.Value.x, 0, pos.Value.z));
                            Handles.color = Color.green;
                            Handles.DrawWireCube(new Vector3(hit.point.x, f1, hit.point.z), new Vector3(0.2f, 0.2f, 0.2f));
                        }
                    }
                }
            }
