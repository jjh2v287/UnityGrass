...
public class GrassPainter : MonoBehaviour
{
    ...
    public void CreateMesh()
    {
        ...
        {
            Vector2 TerrainSize = new Vector2(100f, 100f);
            int cellCountRow = 5;
            int cellCountCol = 5;
            int rows = (int)TerrainSize.x * cellCountRow;
            int cols = (int)TerrainSize.y * cellCountCol;
            float minX = -50.0f;
            float maxX = 50.0f;
            float minZ = -50.0f;
            float maxZ = 50.0f;

            float[] array = new float[] { 0.0f, 0.1f, 0.15f, 0.2f, 0.25f, -0.1f, -0.15f, -0.2f, -0.25f };
            ...
            for (int row = 0; row < rows; row++)
            {
                ...
                {
                    int ranIndexX = rand.Next(array.Length);
                    int ranIndexY = rand.Next(array.Length);
                    float posX = minX + col * stepX + array[ranIndexX];
                    float posZ = minZ + row * stepZ + array[ranIndexY];
                    Vector3 grassPosition = new Vector3(posX, 0f, posZ) + gameObject.transform.position;

                    positions.Add((grassPosition));
                    indicies.Add(indiciesIndex);
        ...
        /*{
            ...
            int cellCountRow = 2;
            int cellCountCol = 2;
            int rows = (int)TerrainSize.x;
            int cols = (int)TerrainSize.y;
            float minX = -50.0f;
            float maxX = 50.0f;
            float minZ = -50.0f;
            float maxZ = 50.0f;
            ...
            float stepX = (maxX - minX) / (cols);
            float stepZ = (maxZ - minZ) / (rows);
            for (int row = (int)minX; row < (int)maxX; row++)
            {
                for (int col = (int)minZ; col < (int)maxZ; col++)
                {
                    for (int cellX = 0; cellX < cellCountRow; cellX++)
                    {
                        for (int cellY = 0; cellY < cellCountCol; cellY++)
                        {
                            int ranIndexX = rand.Next(array.Length);
                            int ranIndexY = rand.Next(array.Length);
                            float posX = col + (stepX * cellX) + array[ranIndexX];
                            float posZ = row + (stepZ * cellY) + array[ranIndexY];
                            Vector3 grassPosition = new Vector3(posX, 0f, posZ) + gameObject.transform.position;
                    ...
                    float centerPosX = col + (0.5f);
                    float centerPosZ = row + (0.5f);
                    Vector3 centerPosition = new Vector3(centerPosX, 0f, centerPosZ) + gameObject.transform.position;
        ...
        if ((Selection.Contains(gameObject)))
        {
            ...
            if (e.type == EventType.MouseDrag && e.button == 1 && toolbarInt == 0)
            {
                ...
                for (int k = 0; k < density; k++)
                {
                    ...
                    if (Physics.Raycast(ray, out terrainHit, 200f, hitMask.value) && indiciesIndex < grassLimit && terrainHit.normal.y <= (1 + normalLimit) && terrainHit.normal.y >= (1 - normalLimit))
                    {
